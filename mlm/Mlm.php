	...
	public function level()
{
	...
	$this->session->set_userdata('z', '447');
	$this->db->where('ref_id', $this->session->userdata('z'));
	$query = $this->db->get('mlm');
	$row =  $query->row();
	...
	if ($query->num_rows() < 2)
	{
		...
		$this->db->where('his_id', $this->session->userdata('z'));
		$this->db->set('level', '1', FALSE);
		$this->db->update('mlm');

		$this->level2($query->result_array());
	}
	...
	if($sum = 4)
		{
			...
			$this->db->where('his_id', $this->session->userdata('z'));
			$this->db->set('level', '2', FALSE);
			$this->db->update('mlm');

			foreach ($data as $value) {
				$this->db->where('ref_id', $value['his_id']);
				$query = $this->db->get('mlm');
				$this->level3($query->row(), 8);
	...
	if($sum = 4)
		{
			...
			foreach ($data as $value) {
				$this->db->where('ref_id', $value['his_id']);
				$query = $this->db->get('mlm');
				//$this->level3($query->result_array());
...
function start(){
$this->session->set_userdata('u', '447');
